...
const SearchPages = ({pageCount}) => {
  ...
  const pageList = arrOfCurrButtons.map((page, index) => {
    return (
      <button
        key={index}
        className={`btn btn-secondary ${currentButton === page ? "active" : ""}`}
  ...
  return (
    <div>
      <div className="btn-group">
        <button className={`btn btn-secondary ${currentButton === 1 ? "disabled" : ""}`}
                onClick={() => setCurrentButton((prev) => prev === 1 ? prev : prev - 1)}>
          Prev
        </button>
        {pageList}
        <button className={`btn btn-secondary ${currentButton === numberOfPages.length ? "disabled" : ""}`}
